...
## 38. P0-EIPB Phase 4.2 — SOAK-2H 混合负载漂移:饱和问题判决(2026-08-26)

### 38.1 执行概况
零 harness 改动,`EIPB_SOAK_SECS=7200`,乱序 C-D-A-B 单 shuffle 连续四块,2026-08-26 11:58–20:00 一条流水跑完( 墙钟 8.03h )。每构建一条连续 120 分钟会话:ops C 52710 / D 54886 / A 55384 / B 53425,cycles 1696/1766/1782/1719,各占满 40×180s 窗口,wall_min 120.0–120.1。四块 teardown 全链( kill-buffers→delete-tree→done→exit-clean )+ `EIPB4_RUN_DONE` 落地;日志内零 FATAL/error/leftover。原始数据 results/eipb_t42.log(+ rss.csv 60s 侧车),判定书 bench/eipb/phase4/report/PHASE4_2.md。

### 38.2 判定
- **核心问题判决:T4.1 的打字漂移在 2 小时尺度不饱和**。type-c p50 末段平台:B ~344 / C ~295 / D ~70 / A ~65 ms;末三分之一斜率 B +6.912、C +5.421 ms/min **加速**,A +0.557 缓升,D +0.352 平斜( 唯一准稳态;type-el 同形,D 判 SATURATING 平台 12.7ms )。"预热/稳态"解释被否定,**长会话退化候选在 ≥2h 尺度成立**( 窗口外无界性仍未证 );
- **方向再证与 ENCA 无关**:禁用 B 是最差终态且加速最猛;唯一准稳态恰是 full-ENCA D。§26 冻结不变,不得据此动架构;
- G2 四构建过:RSS 斜率 71–96 KB/h 无起飞;memory-limit 四构建同步棘轮 +23–24MB( D 同样棘轮但延迟准稳态,B 斜率与 A 相当而漂移斜率 ~12× A )→ 漂移与内存压力在观察层解耦;强制 GC p99 弹跳 43–824ms 无趋势,T1 停顿行 2h 尺度稳定;
- G3 四构建 exit-clean:**p32 挂死累计 8/8 soak 退出未复现**( 含 T4.1 ),tty 域维持清除、GUI 保留标记;
- G1 型尾放大:ta/overall D 1.26 过;C 3.51 / A 3.75 / B 5.88 记 FAIL-as-measured。与 T4.1 不同,**绝对尾不再收敛于单一带**( head/tail p99 ms:C 338→1189、A 109→409、B 105→619、D 395→498 ),T4.1 的"同一终态带"论证在 2H 不可用 → 记 UNRESOLVED( 非 UNRESOLVABLE ),归学说 3 多会话议程;
- 观察层新记录:C 在 w31–32( 约 93 分钟 )每窗 GC 数 ~10× 尖峰( 552→1750/1774 )后坍缩至 <200,同期池化中位短暂回落( 57→12.5ms )后打字曲线再入 190–300ms 加速段;GC 率坍缩的两构建( C 959→56、B 3402→80 )正是打字加速的两构建( A 维持 ~1500–3200/窗缓升,D ~700–1900/窗稳 )。仅作共观察登记,堆增长阈值交互假说未验证,doctrine 8 不满足。

### 38.3 边界
- 会话位置与构建身份在本设计下不可分离( soak 连续性要求单 shuffle ):头部基线不对称( win01 池化 p50 C/D ~8ms vs A/B ~2.4ms;type-c w1 16 vs 3.9 )同样位置混淆,单次读数不作归因;D 的准稳态同理不可判定为构建性质;
- 小幅值类高比值沿用 T4.1 先例按次序统计噪声处理( B imenu 13.48,绝对值 0.8→11.3ms );xref 首桶 p99 跨构建 140–1212ms( T4.1 全体平坦 ~1200–1400 )表明 grep 子进程拉起成本随台架状态波动,mapped 行不受影响;
- Atlas 至此:typing drift 行升级为 v1.2 —— 唯一确认随时间恶化的行( ≥2h 非饱和,禁用 B 最差 );其余 stall 行有界、关闭行维持关闭。Phase 5(T5 外部 IDE 同 trace 对比)是主线剩余项。
